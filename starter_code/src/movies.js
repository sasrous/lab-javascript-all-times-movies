...
function turnHoursToMinutes(arr){
  var cloneArray = copy(arr)
  arr.forEach(function(currentValue, index, array) {
  timeString = currentValue.duration
  var regex = /\d+/g;
  timeArray = timeString.match(regex);  // creates array from matches ( no letters just numbers )
  minutes = parseInt(timeArray[0]*60) + parseInt(timeArray[1]);
  cloneArray[index].duration = minutes;
  });
...
function ratesAverage(arr){
  ...
  sumArray.push(arr[i].rate*100)
  }
  
  var sum = sumArray.reduce( function(total, amount){

    return parseInt(total) + parseInt(amount)
    
  });
 
  sum = sum/sumArray.length;
...
function dramaMoviesRate(arr){
  var dramaArray = copy(arr);
  for (i = dramaArray.length-1; i >= 0; i--) { 
  
  var genreStr = dramaArray[i].genre;
  genreStr = genreStr.join();
  if ( genreStr.search(/drama/i) === -1){
    dramaArray.remove(i);
    ;}}
  var dramaAverage = ratesAverage(dramaArray)
  return dramaAverage;
}
...
return arr.sort(sortBy("duration"));}
orderByDuration(movies);

// How many movies did STEVEN SPIELBERG  

function howManyMovies(arr){
  function checkDirector (currentValue, index, array){
    var genreStr = array[index].genre;
    genreStr = genreStr.join();
    if ( (array[index].director.search(/spielberg/i) != -1) && (genreStr.search(/drama/i) != -1) )
    return array[i]
  }
  return ("Steven Spielberg directed " + arr.filter(checkDirector).length +" drama movies!")
} 
howManyMovies(movies);

// Order by title and print the first 20 titles
function orderAlphabetically(arr){
var titleArray = arr.sort(sortBy("title"));
var sortedByTitle = [];
for (i=0; i < 20; i++){
sortedByTitle.push(titleArray[i].title);
}
